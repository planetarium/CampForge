#!/usr/bin/env bash
# Remote installer for 9c-backoffice camp (run on OpenClaw or any agent workspace)
...
curl -sL "$BASE/packages/9c-backoffice/skills/9c-backoffice/SKILL.md" -o "$WS/skills/9c-backoffice/SKILL.md"
curl -sL "$BASE/packages/9c-backoffice/skills/9c-backoffice/references/api-endpoints.md" -o "$WS/skills/9c-backoffice/references/api-endpoints.md"
for f in check-deleted-addresses sheet-compare sheet-list table-patch-purge-cache table-patch-sign table-patch-stage table-patch-tx-result table-patch-upload-r2 table-patch-validate; do
  curl -sL "$BASE/packages/9c-backoffice/skills/9c-backoffice/queries/${f}.gql" -o "$WS/skills/9c-backoffice/queries/${f}.gql"
done

echo "9c-backoffice camp installed (with gql-ops)"
