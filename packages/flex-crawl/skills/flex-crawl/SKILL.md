...
2. **인증 상태 확인** — `flex-ax status`
3. **크롤링 실행** — `flex-ax crawl`
4. **DB 변환** — `flex-ax import`
5. **결과 리포트** — 템플릿/인스턴스/근태 각 건수와 에러 요약
...
# 비대화식 크롤링 (FLEX_EMAIL / FLEX_PASSWORD env 사용 가능)
flex-ax crawl
...
- crawl + import 모두 성공
- 인증 실패 → `flex-ax login` 재실행 또는 credentials 환경변수 확인 안내
