...
- `package.json`에 새 스킬 의존성 추가 (skillpm이 자동 해석)
- `install.sh`에 tarball URL 추가
- 새 스킬에 외부 CLI 의존성이 있으면 `scripts/install-common.sh`에 공통 함수를 추가하고 `install.sh`에서 호출

### Step 5: 확인

- `manifest.yaml`의 `skills.optional`에 자동 추가됨
- `skills/{skill-id}/SKILL.md` 파일 생성 확인
- `install.sh`에 tarball URL 추가 확인
